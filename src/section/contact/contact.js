  ...
  ({ prevPos, currPos }) => {
   if(Math.abs(currPos.y) > 1150){
     if(!document.getElementById("titleContact").classList.contains("SUUC")){
      ...
      if(!document.getElementById("tel").classList.contains("SUUC")){
        document.getElementById("tel").classList.add("SUUC");
      }
          ...
          <div className="content">
            <div className="icontent">
             ...
             <form onSubmit={sendEmail}>
               <input type="name"  id="nombre" name="nombre" placeholder="Nombre"  required onChange={handleChange}/>
               <input type="email" id="email" name="email" placeholder="Mail" required />
               <input type="tel" id="tel" name="Telefono" placeholder="Telefono" onChange={handleChange} pattern="+54 9 11 [0-9]{4}-[0-9]{4}"/>
               <input type="name" id="compañia" name="Compañia" placeholder="Compañia" required onChange={handleChange}/>
               <input type="submit" id="consulta"  value="Consulta"/>
             </form>
            </div>
